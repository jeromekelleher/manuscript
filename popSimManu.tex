...
\title{\stdpopsim: a community-maintained standard library of population genetic simulations}
...
extremely compact, and can be processed efficiently using the tskit library
...
The ``catalog'' is the central element of \stdpopsim, and records
 properties of species in a systematic manner.
The catalog current includes up-to-date estimates of
generation time and overall population
size, per chromosome recombination and mutation rates, empirical recombination
maps and demographic models. \Stdpopsim does not perform
any simulations itself, but rather delegates this task to different
simulation engines.

Simulations are defined by combining elements from
the

\Stdpopsim is a Python package with a command line interface (CLI) and
application programming interface (API), and is distributed via the
standard PyPI and conda-forge repositories.


Currently, the coalescent simulator
\texttt{msprime}~\citep{kelleher2016efficient} and forwards-simulator
SLiM~\citep{haller2018tree,haller2019slim} are supported.
Other engines, such as \texttt{fwdpy11}~\citep{thornton2014c++},
that output in \texttt{tskit} format may also be supported in the future.


The most important element
of \stdpopsim is its ``catalog'', which systematically organizes information
about species such as generation times, per-chromosome recombination rates
and demographic models.


any simulations but rather del
of two essential elements: simulation engines and
